...
normal_cols = avg_cols.union(vital_cols).union(data.columns[data.columns.str.contains("doc2vec_")])
...
try:
    ...
    model = AbdPainPredictionMLP(INPUT_DIM, N_CLASSES, layer_size=wandb.config["layer_size"], dropout=wandb.config['dropout']).to(device)
...
#         loss = torch.nn.BCEWithLogitsLoss()
    ...
    multilabel_train(model, loss, opt, train_loader, test_loader, device, wandb, n_epochs=wandb.config['epochs'], scheduler=scheduler)
