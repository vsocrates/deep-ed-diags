...
def load_data(data_fp, fast_dev_run=False):
    ...
    if wandb.config["class_weight_type"]:
        ...
        if wandb.config["class_weight_type"] == "effective_sample":
            ...
            class_weights = (1 / samples_per_cls) * (
                samples_per_cls.sum() / wandb.config["class_weight_inv_lambda"]
            )
    ...
    X_train_input[:, normal_col_idxs] = torch.tensor(
        np.nan_to_num(std_scaler.transform(X_train.iloc[:, normal_col_idxs])),
        dtype=torch.float,
    )
    X_train_input[:, all_other_cols_idxs] = torch.tensor(
        np.nan_to_num(minmax_scaler.transform(X_train.iloc[:, all_other_cols_idxs])),
        dtype=torch.float,
    )
    X_test_input[:, normal_col_idxs] = torch.tensor(
        np.nan_to_num(std_scaler.transform(X_test.iloc[:, normal_col_idxs])),
        dtype=torch.float,
    )
    X_test_input[:, all_other_cols_idxs] = torch.tensor(
        np.nan_to_num(minmax_scaler.transform(X_test.iloc[:, all_other_cols_idxs])),
        dtype=torch.float,
    )
